...
void draw_axes();
void draw_triangle(point3d a, point3d b, point3d c, GLfloat* col);
...
// int m = sqrt(n);
// //we'll generate points in a double loop, each will give m points

// double phi, theta, incr = 2* M_PI / m;

// //radius of the sphere
//  int rad = .5 * WINDOWSIZE;
...
//             p.x = x; p.y=y; p.z=z; 
//   //all the points are in [-rad,rad]. There's nothing wrong with
//   //that, except that the function draw_points() assumes the
//   //points are in [0, WINDOWSIZE]^3. We'll shift the points to
//   //[0,2*rad] so that we can use the same functions to draw them.
//           p.x = (p.x+rad);
//           p.y = (p.y+ rad);
//           p.z = (p.z+ rad);
//           points.push_back(p); 
...
void initialize_points_sphere()
{
	...
	for(int i = 0; i < n; i++)
	{
		...
		p.x = int(rad + rad*sin(phi)*cos(theta));
		p.y = int(rad + rad*sin(phi)*sin(theta));
		p.z = int(rad + rad*cos(phi));
...
void draw_hull(){
	...
	if (hull.size() > 0) {
		for (int i=0; i< hull.size(); i++) {
			...
			//  windowtoscreen(points[i].z));
			//now our origin is at the point: draw the cube
			draw_triangle(points.at(temp.a),points.at(temp.b),points.at(temp.c), red);

			//move local system back to where we were
			// glPopMatrix();
			// cout << "Triangle inserted: " << "(" << temp.a->x << "," << temp.a->y << "," << temp.a->z << ") (" << temp.b->x << "," << temp.b->y << "," << temp.b->z << ") (" << temp.c->x << "," << temp.c->y << "," << temp.c->z << ")" << endl;
			cout << "new triangle inserted: " << endl;
...
void draw_triangle(point3d a, point3d b, point3d c, GLfloat* col)
{
	...
	//glColor3fv(col);
	glBegin(GL_POLYGON);
	glVertex3f(windowtoscreen(a.x), windowtoscreen(a.y), windowtoscreen(a.z));
	glVertex3f(windowtoscreen(b.x), windowtoscreen(b.y), windowtoscreen(b.z));
	glVertex3f(windowtoscreen(c.x), windowtoscreen(c.y), windowtoscreen(c.z));
	glEnd();
}
